...
require XSLoader;
XSLoader::load( 'HTTP::Parser::Node', $VERSION );
...
sub new {
	...
	$self->{parser} = HTTP_REQUEST == $self->{type} ? $self->HTTP::Parser::Node::_new_request_http_parser : $self->HTTP::Parser::Node::_new_response_http_parser;
	return $self;
}

sub DESTROY {
	my( $self ) = @ARG;
	HTTP::Parser::Node::_free_parser( $self->{parser} );
	return;
}
...
HTTP::Parser::Node - an HTTP request parser

=head1 SYNOPSIS

  use HTTP::Parser::Node qw(:typical);

=head1 DESCRIPTION

HTTP::Parser::Node is a HTTP parser based on ry's http-parser that can be used either for writing a synchronous HTTP server or an event-driven server.
